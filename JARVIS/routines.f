...
carlos      if ((fold - f) .le. tol*ddum) then
c                                        terminate the algorithm.
carlos         task = 'CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH'
carlos         if (iback .ge. 10) info = -5
...
c                             the truncated Newton step has been used.
	 word = 'TNT'
      ...
      OPEN(UNIT=34,FILE='OTHERNRM')
         WRITE(34,*)sbgnrm
         close(34)
      if (iprint .ge. 99) then
         write (6,*) 'LINE SEARCH',iback,' times; norm of step = ',xstep
c         write (6,2001) iter,f,sbgnrm
...
c         if (imod .eq. 0) write (6,2001) iter,f,sbgnrm
      endif
