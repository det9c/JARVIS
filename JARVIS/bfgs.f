       subroutine bfgs(xyz,natoms,force,rmsmax,input_names,
     $                 maxcyc,f,volu,sout)
     ...
     +               ,x(3*natoms), l(nmax), u(nmax), g(3*natoms),
     $     dsave(29), 
     +    wa(2*mmax*nmax+4*nmax+12*mmax*mmax+12*mmax),rnorm
      integer optx(natoms),opty(natoms),optz(natoms)
      integer          i,k,NCYCLE,ispot,maxcyc,ibestpt
      double precision g2(3*natoms),dummy(3,3),x2(3*natoms),rmsnrm
...
c 
      ...
      write(761,*)''
      write(761,*)' /////////////////////////////////////////////'
      write(761,*)'                   Geometry optimization      '
      write(761,30)'        Convergence tolerance on max force:  ',force
      write(761,30)'        Convergence tolerance on RMS norm  ',rmsmax
...
c         write(761,*)'*****************************************************'
         ...
         if(gmaxc .lt. force)w2='YEP'
          ...
          open(unit=761,file='OUTPUT.DTPOLY',access='append')
         write(761,12)' ',ncycle,'   ',f,'    ',rnorm,rmsnrm,w1,gmaxc,w2
         close(761)
...
c write info to history file
         write(15,*)natoms
         write(15,*)'energy',f,'gradient norm',rnorm,ncycle
         do kk=1,natoms
         jj=3*(kk-1)+1
...
c       write(761,*)'||F||  =',rnorm
       ...
       call space(1)
c          write(761,*)' Stress tensor at convergence  (GPa)'
c          write(761,23)dummy(1,1),dummy(1,2),dummy(1,3)
c          write(761,23)dummy(2,1),dummy(2,2),dummy(2,3)
c          write(761,23)dummy(3,1),dummy(3,2),dummy(3,3)
          call space(2)

c      write(761,*)'Final coordinates in ./bfgs_history.xyz '
      close(761)
